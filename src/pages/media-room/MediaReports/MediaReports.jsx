...
import styles from "./MediaReports.module.scss";
...
const MediaReports = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const response = await getFilteredMediaReleasesByTag("annual-reports", pageNumber, {
          year: mediaYear,
  ...
  return (
    <Layout pageTitle="Annual Reports" backButton={true}>
      <Banner backgroundImage={mediaBackgroundImg} title="Annual Reports" />
      <div className={styles.search}>
        <SearchInput
          ...
          placeholder="Search annual reports by keyword"
              ...
              .map((post, index) => (
                <div key={post.slug + index} className={styles["release-wrapper"]}>
                  <MediaItem
                    ...
                    key={post.slug}
                    page="media"
                    mediaUrlSection="annual-reports"
                    activePost
                  />
                </div>
              ))}
